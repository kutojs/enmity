try {
  ...
  alert(` failed to initialize: ${error.message}`), console.error(error);
}
const modules$1 = {
    Constants: { props: ['Endpoints', 'API_HOST'] },
    ...
    Dispatcher: { props: ['_dispatch', 'dispatch'] },
    ...
    AsyncUsers: { props: ['getUser', 'fetchProfile'] },
  ...
  filters = {
    ...
    byTypeName:
      ...
      (c) =>
        !!c &&
        (b
          ? 'function' == typeof c.type && c.type.name === a
          : 'function' == typeof c.default?.type && c.default.type.name === a),
    byDisplayName:
      (a, b = !0) =>
      (c) =>
        !!c &&
        (b
          ? 'function' == typeof c && c.displayName === a
          : 'function' == typeof c.default && c.default.displayName === a),
  },
  getters = [];
...
function getModule(
  k,
  { all: d = !1, traverse: l = !1, defaultExport: m = !0 } = {}
) {
  if ('function' != typeof k) return null;
  let c = [],
    e = function (a, b) {
      try {
        return k(a, b);
  ...
  for (let b in modules) {
    ...
    let g = common.Moment?.locale(),
      i = common.Locale?.getLocale();
    if (
      common.Moment &&
      common.Locale &&
      g !== i &&
      (common.Moment.locale(i), h && g && h !== g)
    ) {
      blacklist.push(b);
      continue;
    }
...
function get$2(a, b) {
  let c = patches.find((c) => c.mdl === a && c.func === b);
  return c || push(a, b);
}
function patch(d, c, b, e, a = Type.After, h = !1) {
  if (d && 'string' == typeof d) {
    if (c && ['function', 'object'].includes(typeof c)) {
    ...
    g = {
      ...
      unpatch() {
        ...
        if (
          ...
          return;
        let d = patches.findIndex((a) => a.mdl == c && a.func == b);
        d && (patches[d]?.unpatch(), patches.splice(d, 1));
      },
    };
...
function create(a) {
  return {
    getPatchesByCaller,
    before: (b, c, d, e = !1) => before(a, b, c, d, e),
    instead: (b, c, d, e = !1) => instead(a, b, c, d, e),
    after: (b, c, d, e = !1) => after(a, b, c, d, e),
...
const assets = {};
try {
  Patcher$4.after('boofcord-assets', Assets$3, 'registerAsset', (c, [a], b) => {
...
const settings = Settings$3.get('boofcord') ?? {};
...
const Events$2 = {
    BOOFCORD_GET_SETTING: ({ file: a, setting: b, defaults: c }) =>
      settings[a][b] ?? c,
    BOOFCORD_SET_SETTING({ file: a, setting: b, value: c }) {
      settings[a] || (settings[a] = {}),
        void 0 == c ? delete settings[a][b] : (settings[a][b] = c);
    },
    BOOFCORD_TOGGLE_SETTING({ file: a, setting: b, defaults: c }) {
      settings[a] || (settings[a] = {}),
        void 0 === settings[a][b]
...
store.addChangeListener(
  Lodash.debounce(() => Settings$3.set({ boofcord: settings }), 200)
);
var Manager = { store, getAll, getSetting, get: get$1 },
  React = common.React;
const listeners = {};
function BOOFCORD_SET_SETTING(a) {
  return handleSettingsUpdate({ ...a, type: 'set' });
}
function BOOFCORD_TOGGLE_SETTING(a) {
...
Dispatcher.subscribe('BOOFCORD_SET_SETTING', BOOFCORD_SET_SETTING),
  Dispatcher.subscribe('BOOFCORD_TOGGLE_SETTING', BOOFCORD_TOGGLE_SETTING);
var Settings$1 = {
    connectComponent,
    connectStores,
    unsubscribe,
...
const Logger = getByName$1('Logger'),
  BoofcordLogger = new Logger('Boofcord');
let socket;
function connectWebsocket(host) {
  ...
  let address =
    Boolean(Settings$1.get('boofcord', 'autoConnectWS', !1)) &&
    Settings$1.get('boofcord', 'debugWSAddress');
  (address || host) &&
...
function initialize$5() {
  ...
  (globalThis.nativeLoggingHook = function (b, c) {
    return (
      ...
      BoofcordLogger.log(b),
      a.apply(this, arguments)
    );
  }),
    Settings$1.get('boofcord', 'autoConnectWS', !1) && connectWebsocket();
}
...
function createStore(b) {
  let c = { ...(b ?? {}) },
    ...
    e = new Flux.Store(Dispatcher, {
      [`BOOFCORD_FLUX_${a}_SET`]({ key: a, value: b }) {
...
const Patcher$3 = create('boofcord-commands'),
...
const section = {
  ...
  type: 1,
  name: 'Boofcord',
  icon: 'https://kuto.dev/images/icon.png',
};
try {
  initialize$4();
} catch (e) {
...
function initialize$4() {
  (Commands$1.BUILT_IN_SECTIONS.boofcord = section),
    Patcher$3.after(
      SearchStore.default,
      'getQueryCommands',
      (d, [, , b], a) => {
        if (!(!b || b.startsWith('/'))) {
          for (let c of ((a ??= []), commands))
            if (
              !(
                !~c.name?.indexOf(b) ||
                a.some((a) => a.__boofcord && a.id === c.id)
              )
            )
              try {
                a.unshift(c);
    ...
    Patcher$3.after(Assets, 'getApplicationIconURL', (b, [a], c) => {
      if ('boofcord' === a.id) return section.icon;
    });
}
...
function sendCommand(a, b, d) {
  let c = uuid();
  Linking.openURL(
    `com.hammerandchisel.discord://boofcord?id=${c}&command=${a}&params=${b.join(
      ','
    )}`
  ).then(() => {
    d && (replies[c] = d);
        ...
        return 'string' == typeof c
          ...
          ? React.createElement(
              TouchableOpacity,
              {
                ...
                onPress() {
                  Users.getUser(c.id)
                    ? Profiles.showUserProfile({ userId: c.id })
                    : AsyncUsers.fetchProfile(c.id).then(() => {
                        Profiles.showUserProfile({ userId: c.id });
                      });
                },
              },
...
function Page({ settings: b }) {
  let c = StyleSheet$1.createThemedStyleSheet({
    ...
    a = {
      ...
      Hammer: getIDByName('ic_hammer_and_chisel_24px'),
      Server: getIDByName('ic_server_security_24px'),
    },
    d = HermesInternal.getRuntimeProperties(),
    e = getPlugins(),
    f = listThemes().map((a) => a.name);
  return React.createElement(
    ScrollView,
    null,
    React.createElement(
      ...
      React.createElement(
        ...
        React.createElement(FormRow, {
          ...
          onPress: () =>
            Invites.acceptInviteAndTransitionToInviteChannel('weed'),
        }),
        ...
        React.createElement(FormRow, {
          ...
          trailing: FormRow.Arrow,
          onPress: () => Linking.openURL('https://github.com/ignkuto'),
        }),
        React.createElement(FormDivider, null),
        React.createElement(FormRow, {
          label: 'Twitter',
          ...
          trailing: FormRow.Arrow,
          onPress: () => Linking.openURL('https://twitter.com/ignkuto'),
        })
      ),
      React.createElement(
        FormSection,
        ...
        React.createElement(FormRow, {
          ...
          onPress() {
            Toasts.open({
              ...
              Clipboard.setString(`**Plugins**: ${e.join(', ')}`);
          },
        }),
        React.createElement(FormDivider, null),
        React.createElement(FormRow, {
          ...
          trailing: () =>
            React.createElement(Text, { style: c.debugText }, f.length),
          onPress() {
            Toasts.open({
              content: 'Copied to clipboard',
              source: a.Checkmark,
            }),
              Clipboard.setString(`**Themes**: ${f.join(', ')}`);
          },
        }),
        React.createElement(FormDivider, null),
        React.createElement(FormRow, {
      ...
      React.createElement(
        ...
        React.createElement(FormRow, {
          label: 'Boofcord Version',
          leading: React.createElement(FormRow.Icon, {
            source: { uri: 'https://kuto.dev/images/icon.png' },
          }),
          trailing: () =>
            React.createElement(
              Text,
              { style: c.debugText },
              window.boofcord.version
            ),
          onPress: () =>
            Linking.openURL(
        ...
        React.createElement(FormRow, {
          ...
          leading: React.createElement(FormRow.Icon, { source: a.Hammer }),
          trailing: () =>
            React.createElement(
              Text,
              { style: c.debugText },
          ...
          onPress() {
            Toasts.open({
              ...
              Clipboard.setString(String(d['Bytecode Version']));
          },
        }),
        React.createElement(FormDivider, null),
        React.createElement(FormRow, {
          label: 'Hermes Version',
          leading: React.createElement(FormRow.Icon, { source: a.Server }),
          trailing: () =>
            React.createElement(
              Text,
              { style: c.debugText },
              d['OSS Release Version']
            ),
          onPress() {
            Toasts.open({
              content: 'Copied to clipboard',
              ...
              Clipboard.setString(d['OSS Release Version']);
          },
        })
      )
    )
...
const Patcher$2 = create('boofcord-settings');
...
function patchScreens() {
  Patcher$2.after(Scenes, 'default', (b, c, a) => ({
    ...
    Boofcord: {
      key: 'Boofcord',
      title: 'Boofcord',
      render: connectComponent(Page, 'boofcord'),
    },
    BoofcordPlugins: {
      key: 'BoofcordPlugins',
      title: 'Plugins',
...
function patchSettings$1() {
  let a = getByName$1('UserSettingsOverviewWrapper', { default: !1 }),
    b = Patcher$2.after(a, 'default', (d, e, a) => {
      let c = findInReactTree(
        a,
        (a) => a.type?.name === 'UserSettingsOverview'
      );
      Patcher$2.after(
        c.type.prototype,
        'render',
            ...
            React.createElement(
              ...
              React.createElement(
                ...
                React.createElement(FormRow, {
                  ...
                  onPress: () => void d.push('Boofcord', { navigation: d }),
                }),
                React.createElement(FormDivider, null),
                React.createElement(FormRow, {
                  label: 'Plugins',
                  ...
                  trailing: React.createElement(FormArrow, null),
                  onPress: () =>
                    void d.push('BoofcordPlugins', { navigation: d }),
                }),
                React.createElement(FormDivider, null),
                React.createElement(FormRow, {
                  label: 'Themes',
                  ...
                  trailing: React.createElement(FormArrow, null),
                  onPress: () =>
                    void d.push('BoofcordThemes', { navigation: d }),
                })
              )
            )
          );
...
const Util = getByProps('inspect'),
  ...
  methods = ['error', 'info', 'log', 'warn', 'trace', 'debug'];
var patchConsole = () => {
  for (let a of methods)
    (console[a].__ORIGINAL__ = console[a]),
      (console[a] = (...e) => {
        let b = [];
        for (let c = 0, g = e.length; g > c; c++) b.push(Util.inspect(e[c]));
        let f = '';
        for (let d = 0, h = b.length; h > d; d++) f += `${b[d]} `;
        nativeLoggingHook(f, levels[a] ?? levels.info);
      });
};
const BadgesDomain =
  'https://raw.githubusercontent.com/kutojs/enmity/tree/main/badges/';
!(function (a) {
  a[(a.HOUR = 36e5)] = 'HOUR';
})(Times || (Times = {}));
...
function patchBadges() {
  let a = getByName$1('ProfileBadges', { all: !0, default: !1 });
  for (let b of a)
    Patcher.after(b, 'default', (e, [{ user: f, isBoofcord: d, ...g }], a) => {
      if (d) return;
      let [b, h] = React.useState([]);
      React.useEffect(() => {
        try {
          fetchUserBadges(f.id).then(h);
        } catch (a) {
          console.error(`Failed to request/parse badges for ${f.id}`);
        }
      }, []);
      let c = b.map((a) =>
        React.createElement(
      ...
      return b.length ? (a ? (a.props.badges.push(...c), a) : c) : a;
    });
...
function initialize$3() {
  try {
    ...
    console.log('Failed to patch console: ', a.message);
  }
  try {
    patchSettings();
  } catch (b) {
    console.log('Failed to patch settings: ', b.message);
  }
  try {
    patchTracking();
  } catch (c) {
    console.log('Failed to patch trackers: ', c.message);
  }
  try {
    patchBadges();
  } catch (d) {
    console.log('Failed to patch badges: ', d.message);
  }
}
var CorePatches = { initialize: initialize$3 };
const Channels = getByProps('getLastSelectedChannelId');
try {
  Avatars.BOT_AVATARS.BOOFCORD = 'https://kuto.dev/images/icon512.png';
} catch {}
function sendReply(f, b, c, d) {
  let e = f ?? Channels?.getChannelId?.(),
    a = Clyde$1.createBotMessage({ channelId: e, content: '' });
  (a.author.username = c ?? 'Boofcord'),
    (a.author.avatar = d ? c : 'BOOFCORD'),
  ...
  utils = [
    {
      ...
      execute(d, c) {
        let a = [],
          b = HermesInternal.getRuntimeProperties();
        a.push('**Debug Info:**\n'),
          a.push(` > ** Boofcord Version:** ${window.boofcord.version}`),
          a.push(` > ** Discord Version:** ${version}(Build ${build})`),
          a.push(` > ** Hermes Version:** ${b['OSS Release Version']}`),
          a.push(` > ** Bytecode Version:** ${b['Bytecode Version']}`),
          a.push(` > ** Device:** ${device}`),
          a.push(` > ** System:** ${os}`),
          Messages.sendMessage(c.channel.id, {
...
function initialize$1() {
  (window.boofcord = API), Image.prefetch('https://kuto.dev/images/icon.png');
}
